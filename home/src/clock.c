...
static void __watch_added(void *data, Evas_Object *obj, void *event_info)
{
	...
	if (clock_info_s.cur_watch) {
		...
		if (clock_info_s.priv_watch_control) {
			app_control_send_terminate_request(clock_info_s.priv_watch_control);
			app_control_destroy(clock_info_s.priv_watch_control);
			clock_info_s.priv_watch_control = NULL;
		}
	} else {
