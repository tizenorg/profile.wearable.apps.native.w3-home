...
item_info_s *apps_item_info_create_rotary(const char *appid)
{
	...
	//goto_if(PMINFO_R_OK != pkgmgrinfo_appinfo_is_removable(appinfo_h, &removable), ERROR);
	...
	/*if(!strncmp(appid, W_CALENDAR_APP_ID, strlen(W_CALENDAR_APP_ID))) {
		char calendar_icon_path[255] = {0,};
		char *date = NULL;
		char *lang = NULL;
		...
		_D("langset : %s", lang);
		date = _get_date();
		bool is_arabic = _is_arabic(lang);
		if (is_arabic) {
			snprintf(calendar_icon_path, sizeof(calendar_icon_path), SCHEDULE_ICON_ARABIC, date);
		}
		else{
			snprintf(calendar_icon_path, sizeof(calendar_icon_path), SCHEDULE_ICON, date);
		}
		item_info->icon = strdup(calendar_icon_path);
		free(date);
		goto_if(NULL == item_info->icon, ERROR);
	}
	else {
...
//	recent_item_info = apps_recent_info_create_rotary("com.samsung.w-taskmanager");
...
/*
static key_cb_ret_e _home_key_cb1(void *data)
{
	Evas_Object *win = (Evas_Object *)data;
	evas_object_hide(win);
	key_unregister_cb(KEY_TYPE_ROTARY, _home_key_cb1);
...
static key_cb_ret_e _home_key_cb(void *data)
{
	...
	key_unregister_cb(KEY_TYPE_ROTARY, _home_key_cb);
	return KEY_CB_RET_CONTINUE;

}*/
static key_cb_ret_e _back_key_cb3(void *data)
{
	Evas_Object *win = (Evas_Object *)data;
	evas_object_show(win);
	//Evas_Object *scroller = evas_object_data_get(win, PRIVATE_DATA_KEY_ROTARY_ACTIVATION_OBJ);
	//eext_rotary_object_event_activated_set(scroller, EINA_TRUE);
		key_unregister_cb(KEY_TYPE_BACK, _back_key_cb3);
		return KEY_CB_RET_CONTINUE;

}
static key_cb_ret_e _back_key_cb2(void *data)
{
	Evas_Object *win = (Evas_Object *)data;

	evas_object_hide(win);
	//	clock_service_init(win);

		key_unregister_cb(KEY_TYPE_BACK, _back_key_cb2);
		return KEY_CB_RET_CONTINUE;
		//power_mode_ui_init();
}
static key_cb_ret_e _back_key_cb1(void *data)
{
	Evas_Object *win = (Evas_Object *)data;
	evas_object_hide(win);
	//	clock_service_init(win);
		key_unregister_cb(KEY_TYPE_BACK, _back_key_cb1);
		return KEY_CB_RET_CONTINUE;
		//power_mode_ui_init();
}
static void
...
_item_clicked_cb(void *data, Evas_Object *obj, void *event_info)
{
	...
	/* Get current seleted item object */
	item = eext_rotary_selector_selected_item_get(obj);
//	Evas_Object *new_win = elm_win_add(obj,"app_window",ELM_WIN_BASIC);
	/* Get set text for the item */
	main_text = eext_rotary_selector_item_part_text_get(item, "selector,main_text");
	sub_text = eext_rotary_selector_item_part_text_get(item, "selector,sub_text");
	//aul_launch_app(sub_text,NULL);
...
static w_home_error_e _resume_result_cb1(void *data)
{
	Evas_Object *nf =(Evas_Object *)data ;
	eext_rotary_object_event_activated_set(nf, EINA_TRUE);
	main_unregister_cb(APP_STATE_RESUME, _resume_result_cb1);
	//_init_rotary(nf);
	return W_HOME_ERROR_NONE;
	}
/*static w_home_error_e _resume_result_cb2(void *data)
{
	Evas_Object *nf =(Evas_Object *)data ;
	eext_rotary_object_event_activated_set(nf, EINA_TRUE);
	main_unregister_cb(APP_STATE_PAUSE, _resume_result_cb2);
	//_init_rotary(nf);
	return W_HOME_ERROR_NONE;
	}*/
		...
		if(temp == main_get_info()->clock_focus)
		{
			Evas_Object *rotary_selector;
					...
					//	nf_it = elm_naviframe_item_push(nf, _("Rotary Selector"), NULL, NULL, rotary_selector, "empty");
						if (W_HOME_ERROR_NONE != key_register_cb(KEY_TYPE_BACK, _back_key_cb2,new_win)) {
										_APPS_E("Cannot register the key callback");
									}
						if (W_HOME_ERROR_NONE != key_register_cb(KEY_TYPE_BACK, _back_key_cb1,rotary_selector)) {
									_APPS_E("Cannot register the key callback");
								}
						if (W_HOME_ERROR_NONE != key_register_cb(KEY_TYPE_BACK, _back_key_cb3,nf)) {
							_APPS_E("Cannot register the key callback");
						}
						if (W_HOME_ERROR_NONE != main_register_cb(APP_STATE_RESUME, _resume_result_cb1, rotary_selector)) {
								_E("Cannot register the pause callback");
							}


						/*if (W_HOME_ERROR_NONE != key_register_cb(KEY_TYPE_HOME, _home_key_cb1,rotary_selector)) {
									_APPS_E("Cannot register the key callback");
								}
						if (W_HOME_ERROR_NONE != key_register_cb(KEY_TYPE_ROTARY, _home_key_cb,nf)) {
								_APPS_E("Cannot register the key callback");
							}*/
